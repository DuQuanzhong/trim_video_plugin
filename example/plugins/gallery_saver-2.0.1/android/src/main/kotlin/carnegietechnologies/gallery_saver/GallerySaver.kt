...
class GallerySaver internal constructor(private val activity: Activity) :
    ...
    private fun saveMediaFile() {
        uiScope.launch {
            ...
            val success = async(Dispatchers.IO) {
                try {
                    if (mediaType == MediaType.video) {
                        galleryPath = FileUtils.insertVideo(activity, filePath, albumName)
                    } else {
                        galleryPath = FileUtils.insertImage(activity, filePath, albumName)
                    }
                }catch (e:Exception){
            ...
            success.await()
            finishWithSuccess(galleryPath)
        }
    }

    private fun finishWithSuccess(galleryPath: String?) {
        pendingResult?.success(galleryPath)
        pendingResult = null
    }
